librarian::shelf(data.table, ggplot2, fixest, hdm, xgboost, MASS, sandwich, ggthemes, MetBrewer)
...
df_sim1 <- function(n, tau = 1) {
  ...
  out <- as.data.table(cbind(y, w, x))
  setnames(out, c("y", "w", paste0("x", 1:2)))
  out
}
...
df_sim7 <- function(n, tau = 1) {
  ...
  out <- as.data.table(cbind(y, w, z, x))
  setnames(out, c("y", "w", "z", "x"))
  out
}
...
dr_test <- function(df, degree = 1, p = 2) {
  ...
  df.dr <- df[, `:=` (dr = (w - propensity) * (y - cond.mean) / (propensity * (1 - propensity)) + m1 - m0, 
           cond.mean = cond.mean, propensity = propensity)]
  
  if (p == 1) {
    mod.cate <- feols(dr ~ poly(x1, degree, raw = TRUE), data = df.dr)
  } else {
    mod.cate <- feols(dr ~ poly(x1, degree, raw = TRUE) + poly(x2, degree, raw = TRUE), data = df.dr)
  }
...
dr_iv_test <- function(df) {
  mod.pr.z <- feglm(z ~ x, data = df, family = binomial(link = "logit"))
  mod.or.w <- feols(w ~ z, data = df)
  mod.or.y <- feols(y ~ z * x, data = df)
  ...
  df.dr <- df[, `:=`(dr.w = (z - propensity) * (w - cond.mean.w) / (propensity * (1 - propensity)) + m1.w - m0.w, 
...
power.sim1 <- data.table(power.sim1.100, power.sim1.300, power.sim1.1000)[, effect := effect.size] |>
  melt(measure.vars = 1:3, id.vars = 4, variable.name = "N", value.name = "Power")

power.sim2 <- data.table(power.sim2.100, power.sim2.300, power.sim2.1000)[, effect := effect.size] |>
  melt(measure.vars = 1:3, id.vars = 4, variable.name = "N", value.name = "Power")

power.sim3 <- data.table(power.sim3.100, power.sim3.300, power.sim3.1000)[, effect := effect.size] |>
  melt(measure.vars = 1:3, id.vars = 4, variable.name = "N", value.name = "Power")

power.sim4 <- data.table(power.sim4.100, power.sim4.300, power.sim4.1000)[, effect := effect.size] |>
  melt(measure.vars = 1:3, id.vars = 4, variable.name = "N", value.name = "Power")

power.sim <- as.data.table(cbind(power.sim1, power.sim2$Power, power.sim3$Power, power.sim4$Power)) |>
  setnames(c("effect", "N", paste0("design", 1:4))) |>
  melt(measure.vars = 3:6, id.vars = 1:2, variable.name = "Design", value.name = "Power")
power.sim[, N := rep(rep(c(100, 300, 1000), each = 21), 4)]
...
power.sim7 <- data.table(power.sim7.100, power.sim7.300, power.sim7.1000)[, effect := effect.size] |>
  melt(measure.vars = 1:3, id.vars = 4, variable.name = "N", value.name = "Power")

power.sim.alt <- as.data.table(cbind(power.sim1, power.sim2$Power, power.sim3$Power, power.sim4$Power, power.sim7$Power)) |>
  setnames(c("effect", "N", paste0("design", 1:4), "IV")) |>
  melt(measure.vars = 3:7, id.vars = 1:2, variable.name = "Design", value.name = "Power")
power.sim.alt[, N := rep(rep(c(100, 300, 1000), each = 21), 5)]
...
power.sim5 <- data.table(power1.sim5, power2.sim5)[, effect := effect.size] |> 
  melt(measure.vars = 1:2, id.vars = 3, variable.name = "N", value.name = "Power")

power.sim6 <- data.table(power1.sim6, power2.sim6)[, effect := effect.size] |> 
  melt(measure.vars = 1:2, id.vars = 3, variable.name = "N", value.name = "Power")
...
crump_test <- function(df, degree = 1, p = 2) {
  mod0 <- feols(y ~ . - w, data = df[w == 0])
  mod1 <- feols(y ~ . - w, data = df[w == 1])
...
power.comparison1 <- data.frame(crump.sim1.300, power.sim1.300)[, effect := effect.size] |>
  melt(measure.vars = 1:2, id.vars = 3, variable.name = "method", value.name = "Power")

power.comparison2 <- data.frame(crump.sim2.300, power.sim2.300)[, effect := effect.size] |>
  melt(measure.vars = 1:2, id.vars = 3, variable.name = "method", value.name = "Power")

power.comparison3 <- data.frame(crump.sim3.300, power.sim3.300)[, effect := effect.size] |>
  melt(measure.vars = 1:2, id.vars = 3, variable.name = "method", value.name = "Power")

power.comparison4 <- data.frame(crump.sim4.300, power.sim4.300)[, effect := effect.size] |>
  melt(measure.vars = 1:2, id.vars = 3, variable.name = "method", value.name = "Power")

power.comparison <- as.data.table(cbind(power.comparison1, power.comparison2$Power, power.comparison3$Power, power.comparison4$Power)) |>
  setnames(c("effect", "method", paste0("design", 1:4))) |>
  melt(measure.vars = 3:6, id.vars = 1:2, variable.name = "Design", value.name = "Power")
power.comparison[, method := rep(c("Crump et al.", "AIPW"), 21*4)]
...
df_sim_ml1 <- function(n, tau = 1) {
  ...
  out <- as.data.table(cbind(y, w, x))
  setnames(out, c("y", "w", paste0("x", 1:100)))
  out
}
...
df_sim_ml2 <- function(n, tau = 1) {
  ...
  out <- as.data.table(cbind(y, w, x))
  senames(out, c("y", "w", paste0("x", 1:10)))
  out
}
...
dml_test1 <- function(df, wreg = rlasso, yreg = rlasso, nfold = 5) {
  ...
  df.dr <- df[, `:=`(dr = (y - m1) * w / propensity - (y - m0) * (1 - w)/(1 - propensity) + m1 - m0, 
           m1 = m1, m0 = m0, propensity = propensity)]
  
  mod.cate <- feols(dr ~ x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10, data = df.dr)
  beta <- coef(mod.cate)[-1]
  Sig <- vcov(mod.cate)[-1, -1]
  Tstats <- t(beta) %*% solve(Sig) %*% beta
  list(pvalue = 1 - pchisq(Tstats, df = length(beta)), Tstats = Tstats, df.dr = df.dr)
